...
**traverse** map.traverse(callback)

Рекурсивный обход всех элементов карты. Возвращает экземпляр карты. Функция обратного вызова принимает два аргумента: `path` - строковая запись пути к текущему узлу и `node` - текущий узел карты.
